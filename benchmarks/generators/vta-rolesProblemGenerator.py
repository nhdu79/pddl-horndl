...
def generate_planning_problem(obj, trips, filename):
    ...
    while i < len(var_arr)-1:
        planning_domain += "\t\t\t (invoice " + trips_arr[0] + " ?" + var_arr[i] + ")\n \t\t\t (itinerary " + trips_arr[0] + " ?" + var_arr[i+1] + ")\n";
        i += 1
	#~ Chiudo la parentesi di init
    planning_domain += "\t\t\t ) \n \t\t )) \n \t)\n"
...
if __name__ == '__main__':
    trips = 1
    for t in [4,5,6,7,10,15,20,25,30,35,40,45,50,55,60]: #,70,80,90,100]:
        generate_planning_problem(t + 10, trips, filename = "problems/tripProblem" + str(t) + ".pddl")
