...
@base <http://www.semanticweb.org/alisa/ontologies/2021/3/robot> .\n"""
	...
	for column in range(1,columns):
		...
		planning_domain += "\t\t\t\t)\n"
		planning_domain += "\t\t\t(when (and (mko (LeftOf"+str(column)+" ?x)) (not (mko (LeftOf"+str(column+1)+" ?x))))\n"
	...
	for row in range(1,rows):
		...
		planning_domain += "\t\t\t\t)\n"
		planning_domain += "\t\t\t(when (and (mko (BelowOf"+str(row)+" ?x)) (not (mko (BelowOf"+str(row+1)+" ?x))))\n"
...
def generate_planning_problem(rightOf, leftOf, aboveOf, belowOf, column, row, filename):
	...
	#~ Definisco il goal
	planning_domain += "\t(:goal (and (Column" + str(column) + " robot) (Row" + str(row) + " robot)))\n"
