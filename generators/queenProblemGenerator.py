...
def generate_planning_problem(board_size, queen_number, filename):
    ...
    for i in range(board_size):
        for j in range(board_size-1):
             planning_domain += "\t\t(horinline " + board_arr[i*board_size+j] + " " + board_arr[i*board_size+(j+1)] + ")\n"
             planning_domain += "\t\t(verinline " + board_arr[i+j*board_size] + " " + board_arr[i+(j+1)*board_size] + ")\n"
    for i in range(board_size-1):
        for j in range(board_size-1):
             planning_domain += "\t\t(backdiagonal " + board_arr[i*board_size+j] + " " + board_arr[(i+1)*board_size+(j+1)] + ")\n"
        for j in range(1,board_size):
             planning_domain += "\t\t(rightdiagonal " + board_arr[i*board_size+j] + " " + board_arr[(i+1)*board_size+(j-1)] + ")\n"
...
if __name__ == '__main__':
    ...
    for board_size in range(5,16):
        ...
        if board_size < 27:
            ...
            generate_planning_problem(board_size, queen_number, filename = "problems/problem" + str(board_size) + ".pddl")
