#!/usr/bin/env python3
"""Generator for the robotConj benchmark (Horn DL-Lite fragment).
...
Grid semantics (0-indexed, origin at bottom-left):
  ...
  Column_i    — robot is in exactly column i     (= LeftOf_{i+1} ∩ RightOf_i)
  AboveOf_i   — robot is in row ≥ i
  BelowOf_i   — robot is in row < i
  Row_i       — robot is in exactly row i        (= BelowOf_{i+1} ∩ AboveOf_i)
...
BASE_URL = "http://www.semanticweb.org/anon/ontologies/2021/3/"
PAD = " " * 58  # fixed continuation indent for Turtle property lines

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DEFAULT_OUT_DIR = os.path.join(
    SCRIPT_DIR, "..", "benchmarks", "inputs", "horn", "robotConj"
)
...
def _class_plain(name):
    """Bare class declaration — no superclass."""
    return f"###  {BASE_URL}{name}\n {_u(name)} rdf:type owl:Class .\n\n\n"
...
def _general_axiom(c1, c2, target):
    """Intersection axiom: c1 ⊓ c2 ⊑ target."""
    return (
        f"[ owl:intersectionOf ( {_u(c1)}\n"
        f"                       {_u(c2)}\n"
        f"                     ) ;\n"
        f"   rdf:type owl:Class ; \n"
        f"  rdfs:subClassOf {_u(target)}] .\n\n"
    )
...
def generate_owl(n, path):
    header_lines = [
        ...
        "",
        f"<{BASE_URL}robot> rdf:type owl:Ontology .",
        "",
        " ################################################################# ",
        " #    Classes",
        " ################################################################# ",
    ...
    # Column and Row classes — plain declarations, no superclass in TBox
    for i in range(n):
        body += _class_plain(f"Column{i}")
    for i in range(n):
        body += _class_plain(f"Row{i}")
    ...
    # General axioms: LeftOf_{i+1} ⊓ RightOf_i ⊑ Column_i
    #                 BelowOf_{i+1} ⊓ AboveOf_i ⊑ Row_i
    body += "################################################################# \n"
    body += "#    General axioms\n"
    body += " ################################################################# \n"
    body += "\n"
    for i in range(n):
        body += _general_axiom(f"LeftOf{i + 1}", f"RightOf{i}", f"Column{i}")
    for i in range(n):
        body += _general_axiom(f"BelowOf{i + 1}", f"AboveOf{i}", f"Row{i}")
...
def generate_problem(n, path):
    """Robot starts at column 1, row range [0, n-1); goal is Column2 and Row1."""
    lines = [
